...
const generateRandom = (size = 6) => {
    ...
    for (let i = 0; i < size; i++) {
        ...
        const randomItem = tempArray.splice(randomIndex, 1)[0];
        cardValues.push(randomItem);
        cardValues.push(randomItem);
    }
    return cardValues;
};
...
const matrixGenerator = (cardValues, size = 6) => {
    ...
    cardValues = fisherYatesShuffle([...cardValues, ...cardValues]);
    //simple shuffle
    cardValues.sort(() => Math.random() - 0);
